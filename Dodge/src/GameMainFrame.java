...
public class GameMainFrame extends JPanel implements Runnable, KeyListener{
	...
	private BufferedImage image = null;		// 화면 프레임 전체 이미지를 담을 것
	...
	private int life = 3;						// 플레이어 목숨 3개
	private final int SPACEBALL = 150;					// 우주배경 구체 수
	...
	private ArrayList<Object> garoList = null;		// 가로로 나오는 구체 리스트
	private ArrayList<Object> garo2List = null;		// 가로로 나오는 구체 리스트
	private ArrayList<Object> spaceList = null;		// 가로로 나오는 우주 배경 리스트
	private ArrayList<Object> seroList = null;		// 세로로 나오는 구체 리스트
		...
		try {
			while(true) {
			    ...
			    if( ball_count > 100 && SSF.stagepoint == 1) {
			    	...
			    	ball_count = 0;
			    	stagecounting = SSF.stagepoint;				// 스테이지를 기억해줌 stagecounting = 1;
			    }
			    ...
			    // 스테이지 3
			    if( ball_count > 250 && SSF.stagepoint == 3) {		// 곳곳에서 팡팡 터진다!!!!
